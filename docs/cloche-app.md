...
Addition: Crowd Support Upvotes (Direct + Post-Directions Review Queue)
  ...
  New Step [upvotes-4] — Change getFeedItems' shuffle within a distance tier from uniform random to weighted-random using weight = 1 + log(1 + upvote_count). This gives upvoted items a statistically higher chance of surfacing within their tier — a soft weighting, not a hard sort override. Status: done — lib/queries.ts's per-tier shuffle now uses an Efraimidis-Spirakis weighted shuffle (weightedShuffleWithRandom, replacing the old uniform shuffleWithRandom): each item draws key = random()^(1/weight) with weight = 1 + log(1 + upvote_count) from the same seeded PRNG, then tiers are ordered by key descending — higher-upvote items skew toward earlier keys on average without ever being guaranteed to sort first. upvote_count added to the ConfirmedMenuItem type (types/index.ts) to support this. Fix (08/18/26): found live — the shuffle was silently producing the same order on every reload despite the seed genuinely varying per request. Cause was upstream in the DB, not this code: confirmed_menu_items (a view defined with mi.*) was last re-created before upvote_count existed on menu_items, and Postgres freezes a view's column list at (re)creation time, so upvote_count never appeared in query results — see cloche-overview.md Section 5.6 and its 08/18/26 Documentation Log entry for the full trace and the view-refresh SQL. Every item's upvote_count came back undefined, upvoteWeight() turned that into NaN, and a NaN sort key for every item collapses Array.prototype.sort to the unshuffled input order regardless of seed. Hardened upvoteWeight() here to coerce a missing/undefined count to 0 instead of propagating NaN, so a future stale-view recurrence degrades to "no weighting," not "no shuffling."
...
Addition: Visible Upvote Arrow (Toggleable) + Arrow Iconography

  New Step [upvote-arrow-1] — Add an AFTER DELETE trigger on votes (decrement_upvote_count) that decrements menu_items.upvote_count by 1 when a vote row is deleted — mirroring the existing AFTER INSERT trigger from upvotes-1, which only handles increment. This is a schema change that belongs in cloche-overview.md first, since it's shared with the pipeline. Status: done — cloche-overview.md Section 5.4 (votes table) and 5.5 (SQL) updated with decrement_upvote_count() / votes_after_delete, floored at 0 via greatest(upvote_count - 1, 0). Both trigger functions are now SECURITY DEFINER with search_path pinned to public, per the reminder that RLS on menu_items otherwise makes the trigger's UPDATE run as the invoking anon role and silently affect zero rows — the increment_upvote_count() function from upvotes-1 was retroactively re-issued (create or replace) with the same fix, since it had the same gap. Documentation Log entry added (08/18/26, later same day). SQL provided below to run manually in the Supabase SQL Editor — safe to re-run whether or not the original unfixed increment trigger was already applied. Also fixed two stale "Section 5.4" cross-references left over from the upvotes-1 renumbering (one in cloche-overview.md's own Foundation Steps, one in its Documentation Log) that should have pointed at 5.6 (the confirmed_menu_items view) — unrelated to this step but caught while editing the same file.

  New Step [upvote-arrow-2] — Add removeUpvote(itemId) to lib/queries.ts: deletes the current anonymous user's vote row (matching menu_item_id + user_id) from votes. This is the only path that can undo an upvote — a deletion, not a soft flag. Status: done — lib/queries.ts's new removeUpvote(menuItemId) reads the current anonymous session's user id (same pattern as upvoteMenuItem from upvotes-3) and issues a real delete on votes filtered by both menu_item_id and user_id, so it can only ever remove that user's own vote on that dish. The actual upvote_count decrement happens via the AFTER DELETE trigger from upvote-arrow-1 reacting to the row's removal — this function only deletes the row. Not wired to any UI yet — that's upvote-arrow-5.

  New Step [upvote-arrow-3] — Add shared per-item upvoted-state tracking: when the feed loads a page of items, query votes for the current anonymous user's own votes among that page's item ids, and use the result to initialize each item's upvoted (filled/unfilled) state. This becomes the single source of truth that the on-card arrow, the double-tap gesture, and the pending-review banner all read from and update, so all three stay visually in sync for a given item regardless of which one triggered the upvote. Status: done (tracking/initialization only — the arrow icon and the read/update wiring into the double-tap gesture and banner are upvote-arrow-4/5/6, not started). lib/queries.ts's new getUserVotedItemIds(menuItemIds) returns the subset of a given id list the current anonymous user has already voted on. Feed.tsx now holds this as component state (upvotedItemIds, a Set<string>) — the single shared source of truth — seeded by calling getUserVotedItemIds() on every page load: after loadMore appends a new batch to `items`, and after refreshFeed replaces `items` outright on pull-to-refresh, each folds its page's voted ids into the set via a small recordVotedItemIds() helper (additive — refreshFeed's page-1 replace doesn't clear prior entries, since a stale id for an item no longer in `items` is harmless). Not yet read by anything, so `upvotedItemIds` (the value, not its setter) currently trips a harmless eslint no-unused-vars warning — expected, and resolves once upvote-arrow-4 starts reading it.

  New Step [upvote-arrow-4] — Add a persistent arrow icon next to the dish name on each feed card, reflecting the shared state from upvote-arrow-3: unfilled (outline) by default, filled with a green color once the item is upvoted by any means (arrow tap, double-tap, or banner). This is the only icon shape used for upvoting anywhere in the app — no hearts or thumbs. Status: done (visual read only — tap-to-toggle is upvote-arrow-5; the double-tap burst and banner icon still use their prior heart/emoji glyphs until upvote-arrow-6). FeedCard.tsx exports a shared UPVOTE_ARROW_PATH constant (a single triangle-plus-stem arrow glyph, reused as-is by later steps rather than redrawn) and now takes an `upvoted: boolean` prop, rendering that arrow next to the item name: fill="none"/text-ink/25 when false, fill="currentColor"/text-upvote (new --upvote green token in globals.css, deliberately distinct from --accent) when true. Feed.tsx passes `upvoted={upvotedItemIds.has(item.id)}` per card from the upvote-arrow-3 state, which is now actually read (the prior harmless eslint no-unused-vars warning on it is gone).

  New Step [upvote-arrow-5] — Wire the arrow icon to toggle: tapping it while unfilled calls upvoteMenuItem (fills it, records the vote); tapping it while already filled calls removeUpvote from upvote-arrow-2 (unfills it, deletes the vote and decrements the count via upvote-arrow-1's trigger). This is the only interaction anywhere in the app that can remove an upvote — double-tapping an already-upvoted item does not remove it, it remains a no-op fill-only gesture per upvotes-3. Status: done — the arrow (FeedCard.tsx) is now a real button: handleArrowClick stops propagation (so it never triggers the card's double-tap gesture underneath it), flips the current `upvoted` prop, calls the parent's onUpvotedChange(item.id, nextUpvoted) immediately — optimistic, matching how the double-tap burst already behaves — then fires upvoteMenuItem or removeUpvote in the background depending on direction. Feed.tsx's new setItemUpvoted is the single place that actually mutates the shared upvotedItemIds Set (add/delete), passed down as onUpvotedChange, so the arrow's own toggle updates the same source of truth upvote-arrow-3 established. Double-tap's own upvoteMenuItem call still doesn't touch this Set — syncing it there is upvote-arrow-6, not this step.
